//2022.7.20 xuxin -> 2022.8.9 xuxin
`include "defines.v"
...
module divider(
    ...
    wire [127 : 0] start_dividend = w ? {64'b0, 32'b0, abs_dividend[31 : 0]} : {64'b0, abs_dividend};
    wire [63 : 0]  quotient_data  = w ? {32'b0, quotient_reg[30 : 0], ~sub_32[32]} : {quotient_reg[62 : 0], ~sub_64[64]};
    wire [64 : 0] remainder_data  = w ? {32'b0, sub_32} : sub_64;
    wire [127 : 0] dividend_data  = w ? (sub_32[32] ? {dividend_reg[126 : 0], 1'b0} : {63'b0,  sub_32, dividend_reg[30 : 0], 1'b0}) :
                                        (sub_64[64] ? {dividend_reg[126 : 0], 1'b0} : {sub_64[63 : 0], dividend_reg[62 : 0], 1'b0});
    always@(posedge clock) begin
        if(reset == 1'b1) begin
            ...
            dividend_reg  <= start_dividend;
        end
        else if(count != 0) begin
            dividend_reg  <= dividend_data;
            quotient_reg  <= quotient_data;
            remainder_reg <= remainder_data;
        end
    end
