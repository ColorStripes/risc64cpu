...
module dcache #(
    ...
    //read
    input wire [`PC_BUS] address,
    input wire [`CACHE_LINE] arbiter_data,

    output wire hit,
    output wire [`DATA_BUS] data,
    //axi
    output wire axi_wr,
    output wire [`CACHE_LINE] axi_stor_data,
    output wire [`PC_BUS] axi_address
    ...
    Mux #(4, 2, 128) MASK (need_mask, mem_size, {
    ...
    assign rep_address = choose_way0 ? {TAG_RAM_WAY0[index][D_TAG-1 : 0], index, 4'b0} : 
                         ...
                         choose_way3 ? {TAG_RAM_WAY3[index][D_TAG-1 : 0], index, 4'b0} : `ZERO_PC;
