...
module Regfile(
	...
	output reg   [`DATA_BUS] r_data1,      //OUT1
	
	input  wire 		  r_ena2,
	input  wire  [4  : 0] r_addr2,
	output reg   [`DATA_BUS] r_data2,       //OUT2

	//difftest
	output wire [`DATA_BUS] regs_o[0 : 31]

);
reg [`DATA_BUS]	regs[0 : 31];
...
// initial set_gpr_ptr(regs);  // rf为通用寄存器的二维数组变量
	...
	generate
		for (j = 0; j < 32; j = j + 1) begin
			assign regs_o[j] = (w_ena & w_addr == j & j != 0) ? w_data : regs[j];
		end
	endgenerate
